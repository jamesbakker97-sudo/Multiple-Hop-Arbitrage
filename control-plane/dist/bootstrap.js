...
async function loadPoolSnapshots(provider, multicall3Address, pools, stablePools, v3Pools, cuMeter) {
...
async function hydrateStableSnapshots(provider, multicall3Address, stablePools, stableSnapshots, cuMeter) {
    ...
    const calls = pollablePools.flatMap(({ pool }) => [
...
async function hydrateV3Snapshots(provider, multicall3Address, v3Pools, v3Snapshots, cuMeter) {
    const v3Interface = new Interface(uniswapV3PoolAbi);
    const calls = v3Pools.flatMap((pool) => [
