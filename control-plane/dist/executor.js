...
export class ExecutorClient {
    ...
    async validateCandidateRouteQuote(candidate, route, effectiveMinProfit) {
    ...
    async quoteRoute(route, amountIn) {
        let amount = amountIn;
    ...
    async quotePreparedSwaps(swaps, amountIn) {
        let amount = amountIn;
    ...
    async estimateGasCostInToken(route, gasCostWei) {
        if (gasCostWei === 0n) {
    ...
    async quoteSwap(swap, amountIn) {
        if (!this.provider) {
            throw new Error("provider missing");
        }
        if (swap.kind === "v2") {
            const router = new Contract(swap.router, ["function getAmountsOut(uint256 amountIn,address[] calldata path) view returns (uint256[] memory amounts)"], this.provider);
            const amounts = await router.getAmountsOut(amountIn, swap.path);
        ...
        if (swap.kind === "v3") {
            ...
            const quoter = new Contract(this.v3QuoterAddress, [
        ...
        for (const record of this.inflight.values()) {
            ...
            const bumped = {
                ...
                maxFeePerGas: tx.maxFeePerGas
                    ? (tx.maxFeePerGas * (10000n + this.replacementBumpBps)) / 10000n
                    : undefined,
                maxPriorityFeePerGas: tx.maxPriorityFeePerGas
                    ? (tx.maxPriorityFeePerGas * (10000n + this.replacementBumpBps)) / 10000n
                    : undefined,
                gasPrice: tx.gasPrice
                    ? (tx.gasPrice * (10000n + this.replacementBumpBps)) / 10000n
                    : undefined,
            };
    ...
    async maybePauseOnEstimatedLoss(reason) {
        if (this.maxCumulativeEstimatedLossWei <= 0n) {
            return;
        }
